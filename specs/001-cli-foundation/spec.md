...
3. **Dado** um comando desconhecido, **Quando** é executado, **Então** exibe: `Comando desconhecido '<nome>'. Execute 'jira --help' para ver os comandos disponíveis.` e sai com código 64 (uso inválido)
...
1. **Dado** a flag global `--json`, **Quando** qualquer comando de listagem é executado, **Então** stdout recebe JSON válido (array ou objeto) e nada mais; mensagens decorativas vão para stderr
...
- O que ocorre quando `JIRA_BASE_URL` tem barra final?
- O que ocorre quando o servidor Jira está inacessível (timeout)?
- O que ocorre quando o servidor retorna erro 403 (sem permissão)?
- O que ocorre quando `--json` é usado num comando puramente de ação (ex: `jira move`)? — emite objeto de resultado `{"ok":true,"key":"..."}`

## Requisitos *(obrigatório)*

### Requisitos Funcionais

- **RF-001**: A ferramenta DEVE ler `JIRA_TOKEN` e `JIRA_BASE_URL` das variáveis de ambiente
- **RF-002**: A ferramenta DEVE falhar rapidamente com mensagem legível se variáveis obrigatórias estiverem ausentes
- **RF-003**: A ferramenta DEVE suportar `JIRA_INSECURE=true` para ignorar validação SSL
...
- **RF-009**: A ferramenta DEVE suportar flag global `--json` que serializa qualquer saída de listagem ou criação como JSON UTF-8 no stdout, mantendo mensagens decorativas em stderr
...
- **RF-012**: Comandos que aceitam `<KEY>` posicional DEVEM ler Keys do stdin (uma por linha) quando stdin não é TTY e o argumento não é fornecido; quando ambos existem, o argumento prevalece
- **RF-013**: Mensagens de erro retornadas pela API Jira DEVEM ser parseadas (campo `errorMessages` / `errors`) e exibidas de forma legível, não como JSON cru
...
- **CS-002**: Toda mensagem de erro é acionável — informa o que fazer em seguida
